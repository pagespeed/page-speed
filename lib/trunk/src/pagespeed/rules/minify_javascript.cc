...
UserFacingString JsMinifier::body_format() const {
  ...
  return _("%(BEGIN_LINK)sMinify JavaScript%(END_LINK)s for the following "
           "resources to reduce their size by %(SIZE_IN_BYTES)s "
           "(%(PERCENTAGE)s reduction).");
}

const char* JsMinifier::additional_info_url() const {
