...
class JsonDomTest : public testing::Test {
 protected:
  void Parse(const std::string& json_text) {
    ...
    json_.reset(static_cast<DictionaryValue*>(value.release()));
    document_.reset(pagespeed_chromium::CreateDocument(json_.get()));
  }

  DomDocument* document() { return document_.get(); }

 private:
  scoped_ptr<DictionaryValue> json_;
