 ...
 */
public final class TestRunner implements NotificationListener {
   ...
   */
  public static void startTest(TabConnection connection, TestRequest testRequest,
                               long testTimeoutMillis, Listener listener) {
    TestRunner runner = new TestRunner(connection, testRequest, listener);
  ...
  static {
    ...
    // the class loader.  This should work if we're running from a JAR file.
    ...
    if (stream != null) {
      ...
      // The class loader approach won't work in some contexts, such as running `ant test` from the
      // command line; this fallback seems to work in such cases.
  ...
  private synchronized void start(long testTimeoutMillis) {
    ...
    this.watchDogTimer.schedule(new TimerTask() {
        public void run() {
          TestRunner.this.expireTest();
        }
      }, testTimeoutMillis);
    // We start by loading about:blank, just to be sure we load the test URL in a predictable way.
  ...
  private synchronized void startLoadingUrl(String url) {
    ...
    // least as of M16).  Simply evaluating "window.location.href=url" seems to work fine, though.
    ...
    this.tab.asyncCall("Runtime.evaluate", params, new AsyncCallback() {
        ...
        public void onError(String message) {
          TestRunner.this.abortTest(message);
        }
      });
  }
  ...
  private synchronized void fetchResourceContent(final String requestId) {
    ...
    this.tab.asyncCall("Network.getResourceContent", params, new AsyncCallback() {
        public void onSuccess(JSONObject response) {
          TestRunner.this.handleResourceContent(requestId, response);
        }
        public void onError(String message) {
          TestRunner.this.abortTest(message);
        }
  ...
  private synchronized void fetchDomTree() {
    ...
    this.tab.asyncCall("Runtime.evaluate", params, new AsyncCallback() {
        public void onSuccess(JSONObject response) {
          TestRunner.this.handleDomTree(response);
        }
        public void onError(String message) {
          TestRunner.this.abortTest(message);
        }
  ...
  private synchronized void handleDomTree(JSONObject result) {
    ...
    try {
      this.testData.setDomString(Json.getString(Json.getObject(result, "result"), "description"));
      this.checkIfDone();
    } catch (JsonException e) {
      this.abortTest("Bad JSON from Chrome: " + e.getMessage());
    }
  ...
  // Called by the watchdog timer task if we fail to complete the test before reaching the timeout.
