...
class HarGenerator {
  ...
  // TODO(mdsteele): Clean this class up, and eventually nuke HARCreator.

  private static final String HAR_VERSION = "1.2";
  private TestData testData;

  /**
   * Create a new HAR generator object.
   * @param testData the test data from which to generate the HAR
   */
  public HarGenerator(TestData testData) {
  ...
  protected JSONObject generateCreator() {
    JSONObject obj = new JSONObject();
    obj.put("name", "HarGenerator.java");
    obj.put("version", "0.3");
    // obj.put("comment", "");
    return obj;
  }
  ...
  protected JSONObject generatePageTimings() {
    JSONObject obj = new JSONObject();
    JSONObject metrics = testData.getMetrics();
  ...
  protected JSONArray generateEntries() {
    ...
    for (JSONObject obj : testData.getDataForHar()) {
